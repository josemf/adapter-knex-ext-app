...
const doModifications = async (args, entryFile, spinner) => {
    ...
    await asyncForEach(Object.values(keystone.adapters), async adapter => {
        ...
        if (!adapter.doModifications) {
            spinner.info(chalk.yellow.bold(`do-modifications requires the Knex Ext adapter`));            
            return;
        }
        try {
            console.log('Executing list modification files...');
            await adapter.doModifications();
        } catch (e) {
    ...
    if (!errors) {
        spinner.succeed(chalk.green.bold(`List modification files executed`));
        process.exit(0);
    }
    process.exit(1);
};
...
module.exports = {
  ...
  `,
    exec: async (args, { exeName, _cwd = process.cwd() } = {}, spinner) => {
        ...
        spinner.start(' ');
        return doModifications(args, entryFile, spinner);
    },
};
